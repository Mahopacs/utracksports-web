...
- **Soccer-flavored language sweep (2026-06-16) — REPORTED, decisions pending:** beyond the literal word "soccer," soccer-flavored terms remain across the site. Clearest brand/positioning candidates to neutralize: index.html:45 hero ("youth match"/"halftime scramble"), how-to.html:31 h1 ("From kickoff to final"), screenshots.html:31 h1 ("Every angle of the match"); plus the meta/og/twitter descriptions name "match"/"field." Concrete feature/screenshot copy (goals, corners, match clock, match timeline, demo match, "goalkeepers marked", field view) was left soccer-specific by the same rule. No edits made to these yet — awaiting Ted's per-item decision.
